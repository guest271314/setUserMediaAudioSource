    ...
    def quic_event_received(self, event: QuicEvent) -> None:
        ...
        if isinstance(event, StreamDataReceived):
            if event.data:
              ...
              if input_data.decode('utf8') == 'get-audio-sources':
                print(input_data.decode('utf8'))
                data = subprocess.run(['pactl', 'list', 'sources'], capture_output=True)
                self.payload = data.stdout
              elif input_data.decode('utf8') == 'get-audio-source-outputs':
                print(input_data.decode('utf8'))
                data = subprocess.run(['pactl', 'list', 'source-outputs'], capture_output=True)
                self.payload = data.stdout
              else:
            ...
            if event.end_stream:
                ...
                self.connection.send_stream_data(response_id, self.payload, True)
                self.payload = b''
...
class QuicTransportProtocol(QuicConnectionProtocol):
    ...
    def process_client_indication(self) -> None:
        ...
        # URL.
        if path.path == '/setUserMediaAudioSource':
